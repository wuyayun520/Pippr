...
class SecondGridOrientation {
    //: static let shared = AppPermissionTool()
    static let shared = SecondGridOrientation()

    /// 获取麦克风权限
    //: func requestMicPermission(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    func currentObject(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    ...
    func handle(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
        //: if #available(iOS 14, *) {
        if #available(iOS 14, *) {
