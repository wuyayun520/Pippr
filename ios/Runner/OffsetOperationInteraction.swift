...
fileprivate let constRootStr:[UInt8] = [0xec,0xeb,0xec,0xf1,0xad,0xe6,0xea,0xe1,0xe0,0xf7,0xbf,0xac,0xa5,0xed,0xe4,0xf6,0xa5,0xeb,0xea,0xf1,0xa5,0xe7,0xe0,0xe0,0xeb,0xa5,0xec,0xe8,0xf5,0xe9,0xe0,0xe8,0xe0,0xeb,0xf1,0xe0,0xe1]

private func withKey(entity num: UInt8) -> UInt8 {
    return num ^ 133
}
...
// __CLOSE_PRINT__
//
//  OffsetOperationInteraction.swift
//  AbroadTalking
//
...
let mainInstallMessage = 80.0
//: let kProgressHUD_cornerRadius = 14.0
let const_serverTitle = 14.0
//: let kProgressHUD_alpha        = 0.9
let dataCapacityKey = 0.9
//: let kBackgroundView_alpha     = 0.6
let main_forgetData = 0.6
//: let kAnimationInterval        = 0.2
let showVersionMsg = 0.2
//: let kTransformScale           = 0.9
let user_systemPath = 0.9

//: open class ProgressHUD: UIView {
open class OffsetOperationInteraction: UIView {
    //: required public init?(coder: NSCoder) {
    public required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: constRootStr.map{withKey(entity: $0)}, encoding: .utf8)!)
    }

    //: static var shared = ProgressHUD()
    static var shared = OffsetOperationInteraction()
    ...
    class func value(superView: UIView?) {
        //: if superView != nil {
        if superView != nil {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                //: ProgressHUD.shared.frame = superView!.bounds
                OffsetOperationInteraction.shared.frame = superView!.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                OffsetOperationInteraction.shared.activityIndicator.center = OffsetOperationInteraction.shared.center
                //: superView!.addSubview(ProgressHUD.shared)
                superView!.addSubview(OffsetOperationInteraction.shared)
            }
            //: } else {
        } else {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                ...
                OffsetOperationInteraction.shared.frame = UIScreen.main.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                OffsetOperationInteraction.shared.activityIndicator.center = OffsetOperationInteraction.shared.center
                //: AppConfig.getWindow().addSubview(ProgressHUD.shared)
                LayoutAroundActivity.margin().addSubview(OffsetOperationInteraction.shared)
            }
        }
        //: ProgressHUD.shared.hud_startAnimating()
        OffsetOperationInteraction.shared.direct()
    }

    //: class func dismiss() {
    class func macroscopical() {
        //: ProgressHUD.shared.hud_stopAnimating()
        OffsetOperationInteraction.shared.titleObserver()
    }
    ...
    private func direct() {
        ...
        DispatchQueue.main.async {
            ...
            self.activityIndicator.transform = CGAffineTransform(scaleX: user_systemPath, y: user_systemPath)
            //: self.activityIndicator.alpha = 0
            self.activityIndicator.alpha = 0
            //: UIView.animate(withDuration: kAnimationInterval) {
            UIView.animate(withDuration: showVersionMsg) {
                //: self.backgroundColor = UIColor(white: 0, alpha: kBackgroundView_alpha)
                self.backgroundColor = UIColor(white: 0, alpha: main_forgetData)
                //: self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                //: self.activityIndicator.alpha = kProgressHUD_alpha
                self.activityIndicator.alpha = dataCapacityKey
    ...
    private func titleObserver() {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: showVersionMsg) {
                ...
                OffsetOperationInteraction.shared.removeFromSuperview()
            }
        }
    }
    ...
    private lazy var activityIndicator: UIActivityIndicatorView = {
        ...
        indicator.bounds = CGRect(x: 0, y: 0, width: mainInstallMessage, height: mainInstallMessage)
        //: indicator.center = self.center
        indicator.center = self.center
        //: indicator.backgroundColor = .black
        indicator.backgroundColor = .black
        //: indicator.layer.cornerRadius = kProgressHUD_cornerRadius
        indicator.layer.cornerRadius = const_serverTitle
...
extension OffsetOperationInteraction {
    //: class func toast(_ str: String?) {
    class func first(_ str: String?) {
        //: toast(str, showTime: 1)
        close(str, showTime: 1)
    }

    //: class func toast(_ str: String?, showTime: CGFloat) {
    class func close(_ str: String?, showTime: CGFloat) {
        ...
        LayoutAroundActivity.margin().addSubview(titleLab)
        ...
        titleLab.center = LayoutAroundActivity.margin().center
